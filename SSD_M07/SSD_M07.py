...
# 売上の予測
    ...
    #plt.rcParams['font.sans-serif'] = [r'C:\Users\ohika\AppData\Local\Microsoft\Windows\Fonts\ipagothic.ttf']

    # 予測結果をJSON形式で返す
    # 棒グラフを生成
    plt.figure(figsize=(10, 6))
    #plt.bar(result['名前'], result['予測売上(円)'])
    plt.bar(range(1, 51), result['予測売上(円)'][:50])
    plt.xticks(rotation=90)
    plt.xlabel('Sales Ranking')
    plt.ylabel('Projected sales (yen)')
    plt.title('Sales Forecast Results')

    # グラフをファイルまたはバッファに保存します
    plot_filename = 'static/forecast_plot.png'
    plt.savefig(plot_filename)
    # グラフをクリアして、Webページに直接表示されないようにします
    plt.clf()
...
def gura():
    dname = request.form.get('year')
    #result,b= yosoku(int(dname))
    return render_template('gura.html',dname=dname)


if __name__ == '__main__':
    app.debug = True
    #app.run(host="192.168.75.1",port=5000)
    app.run(host="10.141.113.26",port=5000)
